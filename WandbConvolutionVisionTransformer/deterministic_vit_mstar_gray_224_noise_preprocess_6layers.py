...
def main_function(
    ...
    scattering_type="speckle",  # Options: "speckle", "blur", "occlusion", "lowpass"
    ...
    if Training:
        ...
        wandb.init(
            ...
            project=f"deter_mstar_{image_size}_epochs_{epochs}_layers_{num_layers}_lr_{lr}_batch_{batch_size}_embed_{embed_dim}_num_kernel_{kernel_num}_kernel_size_{kernel_size}_kernel_stride_{kernel_stride}_dropout_{dropout}") #mode = 'disabled')
    ...
    if Testing:
        ...
        true_x = np.zeros(
            [
                math.ceil(total_test_images / (batch_size)),
                batch_size,
                image_size,
                image_size,
                channels,
        ...
        true_y = np.zeros(
            [math.ceil(total_test_images / (batch_size)), batch_size, num_classes]
        )
        mu_out_ = np.zeros(
            [math.ceil(total_test_images / (batch_size)), batch_size, num_classes]
        )
        ...
        totalteststeps = 0
        for step, (x, y) in enumerate(test_ds.take(batch_size)):
            totaltestimgs += x.shape[0]
            totalteststeps += 1
        print(f'Total test images : {total_test_images}, {totaltestimgs}')
        print(f'Total test steps : {totalteststeps}')
        for step, (x, y) in enumerate(test_ds.take(batch_size)):
            if x.shape[0] != batch_size:
                print(f'Testing data contains non batch size {x.shape[0]}:{batch_size}')
                continue
            y = tf.one_hot(np.squeeze(y).astype(np.float32), depth=num_classes)
            update_progress(step / int(total_test_images / (batch_size)))
            print(f'test size : {total_test_images} : {int(total_test_images / batch_size)}')
            print(f'ASVKK: Shape : {x.shape} vs {true_x.shape}')
