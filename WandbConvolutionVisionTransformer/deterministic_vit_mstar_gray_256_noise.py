...
os.environ["WANDB_API_KEY"] = "9e2315f2bea7767fb85bd713b3c7aeb766db9c34"
...
class Deterministic_ViT(tf.keras.Model):
    def __init__(
        ...
        **kwargs,
        ...
        self.conv_3 = Deterministic_Conv(
            kernel_size=self.kernel_size,
            kernel_num=self.kernel_num * 2,
            kernel_stride=self.kernel_stride,
            padding="VALID",
        )
        self.conv_4 = Deterministic_Conv(
            kernel_size=self.kernel_size,
            kernel_num=self.kernel_num * 2,
            kernel_stride=self.kernel_stride,
            padding="VALID",
        )
        self.conv_5 = Deterministic_Conv(
            kernel_size=self.kernel_size,
            kernel_num=self.kernel_num * 2,
            kernel_stride=self.kernel_stride,
            padding="VALID",
        )
        self.conv_6 = Deterministic_Conv(
            kernel_size=self.kernel_size,
            kernel_num=self.kernel_num * 2,
            kernel_stride=self.kernel_stride,
            padding="VALID",
...
def add_scattering_blur(images, kernel_size=3):
    ...
    kernel = tf.ones((kernel_size, kernel_size, channels, 1)) / (
...
# lospassfilter
def remove_high_frequency(images, cutoff=0.1):
    images_complex = tf.cast(images, tf.complex64)
    fft_images = tf.signal.fft2d(images_complex)
    fft_images_shifted = tf.signal.fftshift(fft_images, axes=[1, 2])
    shape = tf.shape(images)
    h = shape[1]
    w = shape[2]
    y = tf.linspace(-tf.cast(h, tf.float32)/2, tf.cast(h, tf.float32)/2, h)
    x = tf.linspace(-tf.cast(w, tf.float32)/2, tf.cast(w, tf.float32)/2, w)
    Y, X = tf.meshgrid(y, x, indexing='ij')
    dist = tf.sqrt(X**2 + Y**2)
    max_dist = tf.sqrt((tf.cast(h, tf.float32)/2)**2 + (tf.cast(w, tf.float32)/2)**2)
    mask = tf.cast(dist < cutoff * max_dist, tf.complex64)
    mask = tf.reshape(mask, [1, h, w, 1])
    mask = tf.broadcast_to(mask, tf.shape(fft_images_shifted))
    fft_images_shifted_filtered = fft_images_shifted * mask
    fft_images_filtered = tf.signal.ifftshift(fft_images_shifted_filtered, axes=[1, 2])
    filtered_images = tf.signal.ifft2d(fft_images_filtered)
    filtered_images = tf.abs(filtered_images)
    filtered_images = tf.clip_by_value(filtered_images, 0.0, 1.0)
...
def main_function(
    ...
    data_dir = "Cloned_imgs/Padded_imgs"
    ...
    if Training:
        ...
        os.makedirs(PATH)
        wandb.init(
            entity="ka2642-rochester-institute-of-technology",
            project=f"deter_mstar_{image_size}_epochs_{epochs}_layers_{num_layers}_lr_{lr}_batch_{batch_size}_embed_{embed_dim}_num_kernel_{kernel_num}_kernel_size_{kernel_size}_kernel_stride_{kernel_stride}_dropout_{dropout}",
        )  # mode = 'disabled')
            ...
            if Training:
                ...
                if epochs == 1:
                    ...
                    textfile.write(
                        ...
                        + str(np.mean(train_err[epoch]))
                    )
                    textfile.write(
                        "\n Averaged Validation error : "
                        + str(np.mean(valid_err[epoch]))
                    )
            textfile.write("\n---------------------------------")
            textfile.write("\n---------------------------------")
    if Testing:
        ...
        if Scattering_attack:
            ...
            if scattering_type == "speckle":
                ...
                test_path += "prob_{}_patch_{}".format(
                    occlusion_prob, occlusion_patch_size
                )
            ...
            for step, (x, y) in enumerate(test_ds.take(batch_size)):
                y_onehot = tf.one_hot(
                    np.squeeze(y).astype(np.float32), depth=num_classes
                )
                update_progress(step / int(total_test_images / batch_size))
                if scattering_type == "speckle":
                    x_scatter = add_speckle_noise(x, std_dev=scattering_std)
                elif scattering_type == "blur":
                    x_scatter = add_scattering_blur(
                        x, kernel_size=scattering_kernel_size
                    )
                elif scattering_type == "occlusion":
                    x_scatter = add_scattering_occlusion(
                        x,
                    ...
                    x_scatter = x
                predictions = test_on_batch(
                    x_scatter, y_onehot, trans_model=trans_model
                )
                corr = tf.equal(
                    tf.argmax(predictions, axis=-1), tf.argmax(y_onehot, axis=-1)
                )
                accuracy = tf.reduce_mean(tf.cast(corr, tf.float32))
            ...
            print(
                "\nTest Accuracy with scattering attack ({}): {:.4f}".format(
                    scattering_type, avg_accuracy
                )
            )
            with open(full_test_path + "scattering_attack_results.txt", "w") as f:
                ...
                if scattering_type == "speckle":
                    ...
                    f.write(
                        "Occlusion probability: {}, patch size: {}\n".format(
                            occlusion_prob, occlusion_patch_size
                        )
                    )
        ...
        # ------------------- Random Noise Testing ------------------- #
            ...
            for step, (x, y) in enumerate(test_ds.take(batch_size)):
                ...
                corr = tf.equal(
                    tf.math.argmax(mu_out, axis=-1), tf.math.argmax(y, axis=-1)
                )
                accuracy = tf.reduce_mean(tf.cast(corr, tf.float32))
                acc_test[test_no_steps] = accuracy.numpy()
                if step % 100 == 0:
                    print(
                        "Total running accuracy so far: %.3f" % acc_test[test_no_steps]
                    )
                test_no_steps += 1
        ...
        # ------------------- Adversarial Noise Testing ------------------- #
            ...
            if Targeted:
                test_path = "test_results_targeted_adversarial_noise_{}/".format(
                    epsilon
                )
                full_test_path = PATH + test_path
                if os.path.exists(full_test_path):
            ...
            for step, (x, y) in enumerate(test_ds.take(batch_size)):
                ...
                for advStep in range(maxAdvStep):
                    if Targeted:
                        y_true_batch = np.zeros_like(y)
                        y_true_batch[:, adversary_target_cls] = 1.0
                        adv_perturbations_ = create_adversarial_pattern(
                            adv_x, y_true_batch
                        )
                    else:
                ...
                if step % 50 == 0:
                    print(
                        "Total running accuracy so far: %.4f" % acc_test[test_no_steps]
                    )
                test_no_steps += 1
            test_acc = np.mean(acc_test)
...
if __name__ == "__main__":
    ...
    main_function(
        Training=False,
        Testing=True,
        Scattering_attack=True,
        scattering_type="speckle",
        scattering_std=0.1,
    )
    main_function(
        Training=False,
        Testing=True,
        Scattering_attack=True,
        scattering_type="blur",
        scattering_kernel_size=5,
    )
    main_function(
        Training=False,
        Testing=True,
        Scattering_attack=True,
        scattering_type="occlusion",
        occlusion_prob=0.2,
        occlusion_patch_size=7,
    )

    main_function(Training=False, Testing=True,
                  Scattering_attack=True, scattering_type="lowpass", lowpass_cutoff=0.1)
